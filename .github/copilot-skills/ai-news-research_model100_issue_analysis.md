...
| 첫 거래일 반응 | 해당 뉴스 이후 첫 거래일의 종가 등락률 (%). 장 후 발표면 다음 거래일 기준. |
| 다음 거래일 반응 | 첫 거래일 바로 다음 거래일의 등락률 (%) |
| 해석 | 이 이슈가 바스켓 전체에 미친 영향, 스필오버 여부, 지속성 |

선행 이슈 분류:
- **같은 티어 내 선행 이슈**: 1단계에서 정의한 관련주 내부에서의 선행 이벤트.
- **다른 티어 / 다른 섹터 선행 이슈**: 같은 테마이지만 직접 관련주가 아닌 종목에서의 이벤트.

가격 반응 계산 규칙:
- OHLC DB (`ohlc_1d`)에서 `Close` 기준 전일 대비 등락률을 계산한다.
...
| 가격 반응 | 바스켓 내 주요 종목의 당일 등락률 테이블 |
| 의미 | 이 이벤트가 이슈 확산에서 어떤 역할을 했는지 |

어닝 날짜 연동 규칙:
- 각 타임라인 이벤트마다, 해당 시점에서 **가장 가까운 다음 어닝 발표 예정일**을 함께 기록한다.
- 어닝 데이터는 `calendar_events` 테이블에서 `event_type='earnings'`으로 조회한다.
- 이벤트 시점에서 다음 어닝까지의 거리(일수)가 짧으면 그것이 추가 촉매 역할을 할 수 있으므로 명시한다.
- 형식: `다음 어닝: {ticker} {date} ({N}일 후)`.

확산 타임라인에서 식별해야 하는 핵심 시점:
...
티어 판정 보조 기준:
- **파동 횟수**: 확산 타임라인에서 바스켓 3개 이상 종목이 동시 5% 이상 반응한 날의 횟수.
...
- 이벤트 시점에서 가장 가까운 미래 어닝을 찾으려면: `event_at > ? ORDER BY event_at ASC LIMIT 1`.
...
- `SELECT Datetime, Close FROM ohlc_1d WHERE Symbol=? AND Datetime BETWEEN ? AND ? ORDER BY Datetime`
- 전일 대비 등락률: `(Close / prev_Close - 1) * 100`.
