...
// ── SEC Filing pull endpoint ──
const pullSecSchema = z.object({
  mode: z.enum(["recent", "custom"]).optional().default("recent"),
  from: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
  to: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
  tickerConcurrency: z.number().int().min(1).max(10).optional().default(3),
  requestIntervalMs: z.number().int().min(0).max(10_000).optional().default(300),
});

app.post("/api/news/pull-finhub-sec", async (req, res, next) => {
  try {
    const input = pullSecSchema.parse(req.body ?? {});
    const isCustom = input.mode === "custom";
    const isRecent = input.mode === "recent";
    ...
    // Duplicate job guard
    const secJobKey = "sec_filing";
    const existingJobId = activePullJobs.get(secJobKey);
    if (existingJobId) {
      const existingJob = getJob(existingJobId);
      if (existingJob && existingJob.status === "running") {
        res.status(409).json({
          error: `A SEC filing pull job is already running (jobId=${existingJobId}). Wait for it to finish or cancel it first.`,
          existingJobId,
        });
        return;
      }
      activePullJobs.delete(secJobKey);
    }

    // Load tickers from default universe
    ...
    if (isRecent) {
      secAnchorMap = await getSecFilingAnchorMap();
    }

    const jobId = createJob(tickerList.length);
    activePullJobs.set(secJobKey, jobId);
    appendLog(jobId, `Starting SEC filing ${input.mode} pull — ${tickerList.length} tickers`);

    res.json({ jobId });

    // Background job execution
    (async () => {
      const counters = { totalInserted: 0, totalSkipped: 0, totalCompanion: 0 };
      const newItems: Array<{ id: string; tickers: string[]; publishedAt: string }> = [];
      ...
      try {
        ...
        const processOneTicker = async (ticker: string): Promise<void> => {
          try {
            let tickerFrom = effectiveFrom;
            if (isRecent) {
              const anchor = secAnchorMap?.get(ticker.toUpperCase());
              tickerFrom = anchor ? anchor.slice(0, 10) : fallback7d;
            }

            const items = await fetchSecFilingsRaw(ticker, tickerFrom, effectiveTo);

            if (items.length > 0) {
              appendLog(jobId, `  sec_filing ${ticker}: ${items.length} fetched (${tickerFrom}~${effectiveTo})`);
            }

            for (const item of items) {
              // Insert into news_items (dedup via UNIQUE(source, url) where url = "sec-filing://<accession>")
              const inserted = await insertNewsItem({
              ...
              if (inserted) {
                ...
                // Insert companion sec_filings row
                const companionOk = await insertSecFiling(inserted.id, item);
                if (companionOk) counters.totalCompanion++;
              } else {
                counters.totalSkipped++;
              }
            }
          } catch (err: any) {
            console.error(`[pull-finhub-sec] ${ticker}: ${err.message}`);
            appendLog(jobId, `  ⚠ sec_filing ${ticker}: ${err.message}`);
            if (/429|rate.?limit/i.test(err.message ?? "")) {
              throw err; // propagate rate limit for batch retry
            }
          }
        };

        // Simple sequential + concurrency batch processing
        const batchSize = Math.max(1, Math.min(input.tickerConcurrency, tickerList.length || 1));
        for (let i = 0; i < tickerList.length; i += batchSize) {
          if (isJobCancelled(jobId)) {
            appendLog(jobId, `🛑 Cancelled — stopping ticker processing`);
            break;
          }
          const chunk = tickerList.slice(i, i + batchSize);
          const results = await Promise.allSettled(chunk.map((t) => processOneTicker(t)));
          ...
          // Inter-chunk pause
          if (i + batchSize < tickerList.length && input.requestIntervalMs > 0) {
            await new Promise((r) => setTimeout(r, input.requestIntervalMs));
          }
        }
        ...
        let changeMergeResult = { merged: 0, skipped: 0 };
        if (newItems.length > 0 && !isJobCancelled(jobId)) {
          appendLog(jobId, `Merging change% for ${newItems.length} new SEC items...`);
          try {
            changeMergeResult = await mergeChangeForNewItems(newItems, undefined, () => isJobCancelled(jobId));
            appendLog(jobId, `Change merge: ${changeMergeResult.merged} merged, ${changeMergeResult.skipped} skipped`);
          } catch (err: any) {
            console.error(`[pull-finhub-sec] change merger error: ${err.message}`);
            appendLog(jobId, `⚠ Change merge error: ${err.message}`);
          }
        }

        appendLog(jobId, `SEC filing pull complete — inserted=${counters.totalInserted}, skipped=${counters.totalSkipped}, companion=${counters.totalCompanion}`);

        await setLastSuccess("finhub_sec_filing", new Date().toISOString(), {
          mode: input.mode,
          tickerCount: tickerList.length,
          inserted: counters.totalInserted,
          skipped: counters.totalSkipped,
          companion: counters.totalCompanion,
          changeMerged: changeMergeResult.merged,
        });
        ...
        console.error(`[pull-finhub-sec] job ${jobId} fatal error: ${err.message}`);
        failJob(jobId, err.message || "Unknown error");
        activePullJobs.delete(secJobKey);
      }
    })();
