...
- **앱 런타임 SQLite (terminal 백엔드 기본 DB)**
	...
	- 현재 코드 기준 주의:
		...
		- `company_profiles`의 핵심 컬럼은 이제 `[][][]description[][][]`, `[][][]ipo_date[][][]`, `[][][]market_cap[][][]`, `[][][]peers_json[][][]`에 더해 `[][][]float_shares[][][]`, `[][][]float_pct[][][]`, `[][][]outstanding_shares[][][]`, `[][][]institutional_pct[][][]`, `[][][]institutional_source[][][]`, `[][][]insider_pct[][][]`, `[][][]insider_source[][][]`, `[][][]market_cap_source[][][]`, `[][][]float_source[][][]`까지 포함한다. ticker 심볼은 이 테이블의 컬럼이 아니므로 `securities`와 JOIN해서 해석해야 한다. `POST /api/company-profiles/pull-fmp`, `pull-peers`, `pull-market-cap`, `pull-float`, `pull-holders-yahoo`, `pull-ipo-date`, `pull-yahoo`가 이 테이블을 갱신한다.
