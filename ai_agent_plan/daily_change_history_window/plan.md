...
- backend에는 아직 Daily Change History 전용 endpoint가 없다.
- frontend에도 Daily Change History 전용 window type, component, 탭 등록은 아직 없다.
...
- backend 처리 흐름
  ...
  4. market cap min/max filter를 적용한다.
  5. 필터가 적용된 동일 row set 기준으로 `상승 수 / 하락 수 / 보합 수 / 전체 수`를 계산한다.
  6. summary와 rows를 하나의 response로 반환한다.
- frontend 처리 흐름
  1. 새 `Daily Change History Window`가 날짜 선택 state와 market cap min/max state를 가진다.
  2. state 변경 시 backend history API를 호출한다.
  3. 상단 summary card에 상승/하락/보합/전체 수를 보여준다.
  4. 하단 table에 ticker별 `Ticker | Name | Date | Close | Daily Change % | Close From Open % | Market Cap | Industry`를 표시한다.
  5. 빈 결과일 때는 filter 결과 0건 상태를 명확히 보여준다.
...
- `filtered universe`
  - default universe 전체가 아니라, `selectedDate`에 row가 있고 market cap filter 조건을 통과한 ticker 집합.
  - summary와 table은 반드시 이 같은 집합을 사용한다.
- `상승 / 하락 / 보합`
  - 기본값 제안: `daily change% > 0` 상승, `< 0` 하락, `= 0` 보합.
  - `null`은 집계에서 제외하거나 `missing`으로 분리할지 초기 결정이 필요하다.
...
| D5 | 상승/하락 집계의 null row 처리 | 기본 제외, 필요 시 `missingCount` 추가 | OHLC 누락 row를 억지로 상승/하락에 넣지 않기 위해 |
...
- summary count와 table row count가 정확히 같은 filtered dataset 기준인지 확인해야 한다.
...
설명: `Change_1d_Pct`가 null인 row를 summary에서 제외할지 `missing`으로 분리할지 규칙을 고정한다.
완료 조건(눈으로 확인): 상승/하락/보합 집계 기준식이 plan에 적혀 있다.
...
| 세부 단계 | 작업 | 파일 | 검증 | 상태 |
|-----------|------|------|------|------|
| 1-1 | history row 조회용 repository/helper 추가 | `terminal/backend/src/services/ohlcWatchlistRepository.ts` 또는 신규 service | 타입 검사 + 샘플 query 확인 | ⬜ |
| 1-2 | `selectedDate`, `marketCapMin`, `marketCapMax`를 받는 read endpoint 추가 | `terminal/backend/src/server.ts` | endpoint 200/400 확인 | ⬜ |
| 1-3 | summary와 rows를 같은 filtered dataset 기준으로 반환 | `terminal/backend/src/server.ts` 또는 신규 service | response shape 확인 | ⬜ |
| 1-4 | 기본 날짜와 available date 정보 반환 규칙 확정 | `terminal/backend/src/server.ts` | default date 확인 | ⬜ |
...
완료 조건(눈으로 확인): response에 `summary.total`, `summary.gainers`, `summary.losers`, `summary.flat`가 포함된다.
...
| 세부 단계 | 작업 | 파일 | 검증 | 상태 |
|-----------|------|------|------|------|
| 3-1 | 날짜 선택 state와 market cap min/max state 추가 | 신규 component | 정적 분석 0건 | ⬜ |
| 3-2 | summary card 영역 구현 | 신규 component | 렌더링 확인 | ⬜ |
| 3-3 | ticker table 구현 | 신규 component | 렌더링 확인 | ⬜ |
| 3-4 | empty/loading/error 상태 구현 | 신규 component | 상태 전환 확인 | ⬜ |
| 3-5 | localStorage persistence 범위 결정 및 적용 | 신규 component | 상태 복원 확인 | ⬜ |

3-1 목적: 사용자가 조회 기준을 직접 바꾸게 하기 위함.
설명: `selectedDate`, `marketCapMin`, `marketCapMax` 입력을 두고, 값 변경 시 backend API를 다시 호출한다.
완료 조건(눈으로 확인): 날짜 입력과 custom market cap 필드가 있다.
사람 검증(비개발자): 날짜와 시총 범위를 직접 바꿀 수 있다.
흔한 문제/주의: 입력 중간값에 즉시 fetch를 보내면 UX가 튈 수 있어 apply button 또는 debounce 여부를 정해야 한다.

3-2 목적: 사용자가 시장 breadth를 먼저 보게 하기 위함.
설명: `상승 수`, `하락 수`, `보합 수`, `전체 수`를 상단 card로 노출한다.
완료 조건(눈으로 확인): summary card 4개가 보인다.
...
|----|------|--------|----------------|
| D1 | null change row 집계 방식 | 제외 / missingCount 별도 노출 | Step 1, Step 3 |
| D2 | 필터 적용 방식 | 입력 즉시 fetch / Apply 버튼 / debounce | Step 3 |
| D3 | 날짜 선택 범위 UI | 단일 date picker / quick button 병행 | Step 3 |
...
```text
┌─ BLOCKED UNTIL D1 RESOLVED ───────────────────────────────┐
│ null change row를 summary에서 제외할지 별도 missing으로   │
│ 표시할지 결정돼야 summary 숫자와 UI 라벨을 고정할 수 있다. │
└───────────────────────────────────────────────────────────┘
```

### 병렬 트랙 요약

- Track A의 `0-*` 감사가 끝나야 `1-*` backend API 구현을 시작할 수 있다.
- Track B는 backend contract 이름이 크게 흔들리지 않는 한 `1-*` 후반과 병렬 진행이 가능하다.
- Track C는 최소 `1-2`, `1-3`, `2-3` 이후에 붙이는 것이 안전하다.
- Track D는 구현 완료 후 마지막에 수행한다.

### 차단 요약 테이블

| 결정 | 차단 대상 | 선택지 |
|------|-----------|--------|
| D1 | Step 1, Step 3 summary 라벨 | null 제외 / missingCount 추가 |
| D2 | Step 3 fetch UX | 입력 즉시 / Apply 버튼 / debounce |
| D3 | Step 3 date UX | 단일 picker / quick preset 병행 |