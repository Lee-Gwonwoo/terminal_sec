...
def primary_impact_score(row: dict) -> float | None:
    values = [
        abs(value)
        for value in (
            row.get("change_pct"),
...
def supplementary_intraday_score(row: dict) -> float | None:
    value = row.get("change_open_to_high_pct")
...
def build_analysis(rows: list[dict], market_caps: dict[str, float]) -> dict:
    ...
    for row in rows:
        ...
        row["impact_score"] = primary_impact_score(row)
        row["supplementary_intraday_score"] = supplementary_intraday_score(row)
...
def make_markdown(since: str, until: str, analysis: dict) -> str:
    lines: list[str] = []
    lines.append(f"# Model 2 Case Analysis ({since} ~ {until})")
    lines.append("")
    ...
    lines.append("")
    lines.append("## 사고과정 로그")
    lines.append("")
    lines.append("1. 범위는 `2024-01-01`부터 현재까지의 전체 뉴스(`press_release`, `news`, `company_news`, `market_news`, `IBKR`)로 확장했다.")
    lines.append("2. 외부 링크는 다시 열지 않고 `news_items`, `news_fulltext`, `news_change_metrics`, `company_profiles.market_cap`만 사용했다.")
    lines.append("3. 텍스트 분류는 `title/body/full_text` 순으로 사용했고, full text가 없는 source는 `title/body`만으로 case를 분류했다.")
    lines.append("4. 영향 판정은 `change_pct`, `change_1d_pct`, `change_from_open_pct`의 절대값 최대치를 `impact_score`로 두고 진행했다.")
    lines.append("5. 시총 기준은 `$300M-<$1B`, `$1B-<$100B`, `>= $100B` 3개를 주 버킷으로 두고, `<$300M or Unknown`은 보조 집단으로 따로 기록했다.")
    lines.append("6. 각 시총 버킷 안에서 `impact_score`의 p80을 임계값으로 잡아 `영향 미침 / 영향 안 미침`을 분리했다.")
    lines.append("7. 동일 case 안에서 `영향 미침 : 영향 안 미침` 비율과 Wilson lower bound를 같이 사용해 순위를 잡았다.")
    lines.append("")
    ...
    lines.append("")
    lines.append("- 사용한 primary change 컬럼: `change_pct`, `change_1d_pct`, `change_from_open_pct`")
    lines.append("- 사용한 보조 intraday 컬럼: `change_open_to_high_pct` (참고만, threshold 산정에는 미포함)")
    lines.append("- `impact_score = max(abs(change_pct), abs(change_1d_pct), abs(change_from_open_pct))`")
    lines.append("- 방향성은 절대값이 가장 큰 primary change 컬럼의 부호로 결정했다.")
    ...
    for index, summary in enumerate(analysis["case_summaries"][:12], start=1):
        ...
        for example in summary["top_examples"][:3]:
            lines.append(
                f"  - {example['date']} | {example['source_type']} | {example['ticker']} | {example['title']} | change={example['change_pct']} | change_1d={example['change_1d_pct']} | intraday={example['change_from_open_pct']} | cap={example['market_cap_bucket']}"
            )
    ...
    lines.append("- 실적/가이던스, 자금조달, 전략 제휴는 source type이 달라도 반복적으로 나타나는 상위 case다.")
    ...
    lines.append("- 이 note의 사고과정 로그는 절차/기준 로그이며, 내부 추론 전체를 그대로 저장한 것은 아니다.")
    return "\n".join(lines)
...
def main() -> None:
    ...
    markdown = make_markdown(args.since, until, analysis)
    ...
    log_path.write_text(
        "\n".join(
            [
                ...
                f"until={until}",
                "scope=all_news",
                "primary_impact_formula=max(abs(change_pct), abs(change_1d_pct), abs(change_from_open_pct))",
    ...
    if args.page_id:
        update_research_page(conn, args.page_id, f"Model 2 all-news case analysis ({args.since} ~ {until})", markdown)
