...
- **세부 단계 테이블(각 Step마다 필수):**
  ...
  | 세부 단계 | ID (예: `1-1`) |
  | 작업 | 무엇을 하는지 한 문장 |
  | 파일 | 생성/수정하는 파일 (감사/조사형 Step에서는 생략 가능) |
  | 검증 | 해당 세부 단계가 올바르게 완료됐는지 확인하는 정확한 명령/체크 (감사/조사형 Step에서는 생략 가능) |
  | 상태 | 이모지 상태 마커 (✅/⏳/⬜/🚫) — **필수** |

  **포맷 A — 감사/조사형 Step** (파일/검증 컬럼 없음):
  ```
  | 세부 단계 | 작업 | 상태 |
  |-----------|------|------|
  | 0-1 | Finnhub 프로브(profile/news/earnings 등)로 필드 가용성 확인 | ✅ 완료 |
  | 0-2 | IBKR TWS 프로브 v1 | ✅ 완료 |
  ```

  **포맷 B — 구현형 Step** (파일/검증 컬럼 포함):
  ```
  | 세부 단계 | 작업 | 파일 | 검증 | 상태 |
  |-----------|------|------|------|------|
  | 1-1 | `initDb()`에 `foo` 테이블 CREATE 추가 | `src/db.ts` | 백엔드 시작 → 테이블 존재 확인 | ✅ |
  | 1-2 | `fooRepository.ts` 서비스 생성 | `src/services/fooRepository.ts` | `npx tsc --noEmit` → 에러 0개 | ⬜ |
  ```

  > **주의:** `| 상태 |` 컬럼은 포맷 A/B 모두 **필수**. 생략 금지.
...
- **세부 단계 목적/설명 블록(인라인, 권장):**
  ...
  - 예시:
    ```
    세부 단계 목적/설명 (1단계)
    - `1-1` 목적: status를 DB에 영속화. 설명:
      - init 시 `update_status` 테이블을 생성/갱신.
      - 완료 조건(눈으로 확인): DB 쿼리로 테이블 존재(+컬럼) 확인이 된다.
      - 사람 검증(비개발자): 제공된 쿼리를 실행해서 테이블 이름이 보이는지 확인한다.
      - 흔한 문제/주의: DB 경로가 다른 곳을 가리킴; 리팩터링 후 스키마 불일치.
    - `1-2` 목적: 쓰기 로직을 한 곳에 모음. 설명:
      - 라우트에서 사용하는 repository 메서드를 구현.
      - 완료 조건(눈으로 확인): 라우트가 repository를 호출하고, 테스트/TS 빌드가 통과한다.
      - 사람 검증(비개발자): 검증 명령을 실행해서 "에러 0개"인지 확인한다.
      - 흔한 문제/주의: 순환 import; 반환 shape이 호출부마다 달라짐.
    ```

  - **선택 확장 필드:** 세부 단계의 성격에 따라 아래 필드를 추가할 수 있습니다(필수 아님):
    - **성공 기준:** 정량적/기능적으로 "성공"을 정의해야 할 때 (예: API 응답 200 + 결과 ≥1건).
    - **검증 기준:** 완료 조건과 별개로, 검증 절차의 기대 결과를 명시할 때.
...
- **plan.md 작성 규칙(plan.md 생성/수정 시 반드시 준수):**
  - **구조 순서 (실제 plan.md 기준):**
    1. **목표** (`### 목표`)
    2. **현재 레포 상태** (`### 현재 레포 상태(중요, 확인됨)`) — 코드베이스 현황, 기존 구현물 파악
    3. **제약 / 비범위** (`### 제약 / 비범위`) — 이번 plan이 다루지 않는 것
    4. **읽는 방법** (`### 읽는 방법(비개발자/일반인 기준)`) — 비개발자를 위한 문서 탐색 가이드
    5. **프로세스 템플릿** (`### 프로세스 템플릿(plan 변경 + 단계 완료 확인)`) — 리비전·확인 절차 템플릿
    6. **아키텍처(상위)** (`### 아키텍처(상위)`) — 시스템 구성, 폴더 구조, 데이터 흐름
    7. **결정/선행조건** (`### 결정/선행조건(초기에 확정 필요)`) — 구현 전 결정해야 할 사항 + 선택지
    8. **계획 중간 필수 확인** (`### 계획 중간 필수 확인`) — 데이터 수집 가능 범위 점검 등 중간 감사
    9. **제안 구현 순서(이유)** (`### 제안하는 구현 순서(이유)`) — Step 순서 배치 근거
    10. **단계별 계획** (`### 단계별 계획(각 단계: 구현 → 검증)`) — 각 Step (세부 단계 테이블 + 목적/설명 블록 + 검증 훅)
        - 범례(Legend)를 Step 목록 직전에 배치: `> Legend: ✅ 구현+사용자확인 완료 · ⏳ 구현완료, 사용자확인 대기 · ⬜ 미착수 · 🚫 선행조건 미충족(차단)`
    11. **미확정 사항** (`### 미확정 사항(명시 결정 필요)`) — 번호 ID + 선택지 + 차단 대상 Step
    12. **실행 의존성 그래프** (`### 실행 의존성 그래프`) — 상세 ASCII 그래프
    13. **결정 상세** (선택, `### 결정 #N — 제목(상세)`) — 특정 미결정 사항의 옵션 비교/분석

    > 항목 1–3, 10–12는 필수. 나머지(4–9, 13)는 plan 규모/복잡도에 따라 포함 여부를 판단합니다.
    > 항목 순서는 위 번호를 기준으로 유지합니다.
