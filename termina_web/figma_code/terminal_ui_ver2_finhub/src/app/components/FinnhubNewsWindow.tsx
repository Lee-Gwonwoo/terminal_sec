...
export function FinnhubNewsWindow({
  ...
  const industryInstructionMenuRef = useRef<HTMLDivElement>(null);
  ...
  const openIndustryInstructionDialog = useCallback(async (industry: string) => {
    const trimmed = industry.trim();
    if (!trimmed) return;
  ...
  const closeIndustryInstructionDialog = useCallback(() => {
  ...
  useEffect(() => {
    if (!industryInstructionMenu) return;

    const closeMenu = () => setIndustryInstructionMenu(null);
    const onMouseDown = (event: MouseEvent) => {
      if (industryInstructionMenuRef.current && industryInstructionMenuRef.current.contains(event.target as Node)) return;
      closeMenu();
    };
              ...
              <div className="relative" ref={industryMenuRef}>
                ...
                {showIndustryMenu && (
                  <div className="absolute top-full left-0 mt-1 max-h-80 w-72 overflow-auto rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-lg z-30 p-1.5">
                    ...
                    {industryInstructionMenu && (
                      <div
                        ref={industryInstructionMenuRef}
                        className="fixed z-40 w-[188px] overflow-hidden rounded-lg border border-gray-200 bg-white shadow-xl dark:border-gray-700 dark:bg-gray-900"
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                      {industryInstructionDetail.tickers.map((row) => (
                        <tr key={row.ticker} className="hover:bg-gray-50 dark:hover:bg-gray-800/70">
                          ...
                          <td className="px-3 py-2 text-right tabular-nums text-gray-900 dark:text-gray-100">{formatMarketCap(row.marketCap)}</td>
