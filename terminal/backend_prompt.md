...
- 이때 자동 후속 fulltext concurrency는 pull payload의 `[][][]fulltextConcurrency[][][]`를 사용하며, 프론트는 이를 현재 `ft-concurrency` 값으로 채운다. 즉 수동 company fulltext와 자동 company fulltext가 같은 설정값을 공유한다.
...
- FMP PR와 같이 auto fulltext를 수행하지만, 이때 동시성은 별도 fulltext 설정이 아니라 pull payload의 `tickerConcurrency`를 그대로 사용한다.
...
- manual `fmp_stock_news` fulltext는 일반 `ft-concurrency`를 사용한다.
- 반면 `POST /api/news/pull-fmp-press-release`, `POST /api/news/pull-fmp-stock-news` 안에서 새 row에 대해 즉시 실행하는 auto fulltext는 별도 fulltext 설정이 아니라 pull payload의 `tickerConcurrency` worker 수를 사용한다.
- 즉 manual fulltext와 pull 중 auto fulltext는 concurrency source가 다르다.
