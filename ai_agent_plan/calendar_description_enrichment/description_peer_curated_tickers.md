...
- 범위: Calendar description enrichment 작업에서 Batch 001~004까지 description과 peer group을 함께 정리했거나, 이전 pilot 후 batch에서 다시 force refresh한 ticker.
- 상태: 구현/검증 완료, 사용자 화면 확인은 `확인 대기`.
- 중복 처리: 여러 batch에 들어간 ticker는 unique 목록에서 한 번만 표시한다.
- 총 unique ticker 수: 60개.
...
- Batch tag 기준(`industry_override_needed`, `full_peer_curation_batch_002`, `full_peer_curation_batch_003`, `full_peer_curation_batch_004`)으로 peer는 정리됐지만 description이 비어 있는 ticker: 0개.
- 전체 자동 baseline peer graph 기준으로는 peer edge가 있지만 curated description이 아직 없는 ticker가 2,216개다. 이는 전체 default universe를 덮는 자동 peer baseline이며, 이 파일의 `정리 완료` ticker로 보지 않는다.
