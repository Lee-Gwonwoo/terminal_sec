...
- 또한 `Model_1` 최종 주요 이슈 리스트는 기본적으로 **시가총액 `100B` 미만**이고 **`institutional ownership % < 90` 인 ticker만 직접 분석 대상**으로 삼는다. `100B` 이상 ticker나 `institutional 90%+` ticker는 필요하면 reference case 또는 제외 메모로만 적는다.
