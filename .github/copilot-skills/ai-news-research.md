...
2. 먼저 뉴스의 언어적 방향성을 기준으로 `long / short / neutral(or residual)` 상위 분류를 정한다.
...
- `Model_2`의 유형 체계는 `상위 분류(long / short / neutral-or-residual) -> 세부 case_type` 순서로 설계한다.
...
- `neutral` 또는 `residual`은 방향성이 약하거나 혼재되어 있고, long/short 어느 쪽으로도 언어적으로 강하게 단정하기 어려운 뉴스에 붙인다.
- `meaningless_others` 같은 잔여 유형은 보통 이 `neutral/residual` 상위 분류 아래에 둔다.
...
- 유형 분류의 실제 순서는 아래처럼 본다.
  - 1차 질문: 이 뉴스는 언어적으로 `long`, `short`, `neutral/residual` 중 어디에 가까운가?
...
- 위 `10 ~ 50`개는 기본적으로 **세부 case_type 개수**를 의미한다. 상위 `long / short / neutral-residual` 분류는 이 개수와 별도로 먼저 둔다.
- 권장 구조는 `대분류 case_type + 필요시 하위 subtype 메모` 방식이다. 즉 note 본문 순위는 너무 잘게 쪼개지 말고, 세부 차이는 내부 로그나 근거 표 해설에서 보완한다.
- note를 작성할 때는 가능하면 먼저 `long`, `short`, `neutral/residual` 3개 큰 묶음으로 섹션을 나눈 뒤, 각 묶음 안에서 세부 유형을 영향력 순으로 정렬한다.
...
4. `title/body/full_text`를 읽고, **등락률을 보지 않은 상태에서** 뉴스의 언어적 방향성을 기준으로 1차 `long / short / neutral-residual` 상위 분류를 정한다.
...
9. 최종 note는 가능하면 `long -> short -> neutral/residual` 큰 순서로 정리하고, 각 묶음 안에서 영향력 순으로 세부 유형을 배치한다.
...
- 방향이 혼재되거나 정보성 공지에 가까워 언어 기반 판정이 어려우면 `neutral/residual`로 보낸다.
...
- 너무 짧게 `p80 사용`, `press_release만 사용`처럼 끝내지 말고, 아래 수준으로 남긴다.
  - 왜 `long / short / neutral-residual` 상위 분류를 그렇게 정했는지
...
- `long / short / neutral-residual` 상위 분류 정의와 판정 기준
