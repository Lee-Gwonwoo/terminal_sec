...
### 1-6-5-1. Investing.com — 채택 결정 (2026-08-05, 사용자 지시)

앞서 §1-5에서 반려했으나, **미래 일정 기준으로만 판단한 것**이었다. 과거 날짜로 다시 재보니
**다른 소스가 못 하는 두 가지**를 준다.

**과거 날짜 실측 (2026-08-04)**

```
322행 (US 필터) / 티커 추출 320
EPS실적 97%  EPS예상 91%  매출실적 98%  매출예상 88%  세션 98%  시총 100%
ZETA 세션 = "After market close" → SEC EDGAR 8-K 접수 16:05 ET와 일치 ✅
```

| 항목 | Investing | Nasdaq | Yahoo |
|---|---|---|---|
| **과거 날짜의 매출 실적** | ✅ 98% | ❌ 필드 없음 | 티커당 3~4분기만 |
| **과거 날짜의 세션** | ✅ 98% | ❌ 지워짐 | 티커당 4분기만 |

**사용자 결정**
1. ToS 스크래핑 우려 → **이미 investing news를 같은 방식으로 수집 중이므로 동일 기준 적용.** 간격을 두고 순차 요청한다.
2. **실적 데이터는 Investing 기준으로 통일한다.** → `reconcileInvestingEarnings`는 `eps_actual`/`revenue_actual`/`eps_est`/`revenue_est`를 **덮어쓴다** (다른 경로는 빈 칸만 채우는 것과 다름).

**남는 데이터 리스크 (통일해도 해소되지 않음)**

EPS 기준(GAAP vs adjusted)이 **한 행 안에서** 섞이는 경우가 있다.
```
ZETA 2026-08-04  Investing 실적 0.03(GAAP) / 예상 0.1995(non-GAAP)  → 그대로 나누면 대규모 miss로 보임
                 Nasdaq    실적 0.03       / 예상 0.01              → +200% beat (내부 일관)
AMD  2026-08-04  Investing 실적 1.66  vs  Nasdaq 실적 1.44          → adjusted vs GAAP
```
→ **`surprise_pct`를 계산·저장하지 않는다.** 실적/예상 원본 두 값만 저장하고 판단은 화면에서.

**요청 방식 (실측 기반)**

- 넓은 범위 일괄 요청은 **403**. `2026-08-01~08-31` 한 번에 요청 → 즉시 차단됨.
- 따라서 **일자별 순차 + 간격 1,200ms + 지터**(일정 간격 자체가 봇 신호). 병렬 금지.
- 403/429/503 시 지연을 +2초씩 누적(최대 20초)하고 `investingBrowser` 컨텍스트로 폴백 — news 스크래퍼와 동일 구조.
- 비용: 3개월 ≈ 66영업일 × 1.2초 ≈ **1.5분**.

**파싱 안전장치**

예상치 셀에는 클래스가 없어(`<td class="leftStrong">`) 순수 위치 기반은 위험하다.
실적 셀에는 식별자가 있으므로(`pid-...-eps_actual`, `pid-...-rev_actual`) **실적 셀을 앵커로 찾고
예상치는 그 바로 다음 셀**로 잡는다. 앵커가 없으면 행을 버린다.

검증 (2026-08-04 실제 응답 HTML):
```
정상 HTML   → 320행 파싱 (원본 322행 중 티커 링크 없는 2행 제외)
              EPS실적 312 / EPS예상 293 / 매출실적 314 / 매출예상 283 / 세션 315 / 시총 320
컬럼 삽입   → 320행, ZETA 값 그대로 유지 (앵커 정상 동작, 값 섞임 없음)
앵커 클래스 제거 → 0행  (조용히 틀리는 대신 수집량 0으로 드러남)
```
