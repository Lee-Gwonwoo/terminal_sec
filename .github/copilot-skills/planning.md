...
- **`agent_log.md` 언어 규칙(필수):** `agent_log.md`는 **한국어 단독**으로 작성한다.
...
- **`agent_log.md` 순서 규칙(필수):** 새 로그 항목은 **항상 파일 맨 아래에 append**한다.
  - 주제별로 관련 항목 근처에 끼워 넣지 않는다.
  - 로그의 기본 정렬 기준은 **시간순(위=과거, 아래=최신)** 이다.
  - 기존 항목의 상대적 순서는 사용자가 명시적으로 정리/재배치를 요청한 경우가 아니면 바꾸지 않는다.
  - 같은 날 여러 작업을 기록하더라도, 해당 시점의 새 항목을 이전 항목 아래로 계속 누적한다.
- 저장 위치: `C:\github_coding\terminal_sec\ai_agent_plan\<project_name>\`
  - `plan.md`: 작업 시작 전 상세 단계별 계획 (목표, 접근법, 생성/수정 파일, 순서, 위험 요소). **한국어로 작성.**
...
## 자체 검증 필수 규칙 (Mandatory Self-Verification)

> ℹ️ 아래 규칙은 코드 변경이 발생할 때마다 적용됩니다. "빌드 통과"만으로 검증 완료를 선언하지 않습니다.

### 검증 계층 (모두 통과해야 "검증 완료")
코드 변경 후 아래 계층을 **위에서 아래로 순서대로** 모두 수행합니다. 어느 한 계층이라도 실패하면 수정 후 해당 계층부터 재검증합니다.

| # | 계층 | 내용 | 도구/방법 |
|---|------|------|-----------|
| 1 | **정적 분석** | TypeScript/lint 에러 0개 | `get_errors`, IDE 에러 확인 |
| 2 | **빌드** | frontend `npm run build` + backend `npm run build` 모두 성공 | 터미널 실행 |
| 3 | **자동 테스트** | `npm run test` 전체 pass | 터미널 실행 |
| 4 | **런타임 통합** | dev 서버를 띄우고 변경된 기능이 실제로 동작하는지 확인 | 아래 상세 참조 |

### 계층 4 — 런타임 통합 검증 상세

**API/Backend 변경 시:**
- dev 서버(`npm run dev`)를 띄운다.
- 변경/추가한 endpoint에 실제 HTTP 요청(`Invoke-WebRequest` 등)을 보내 응답 코드 + body를 확인한다.
- stub/미구현 endpoint는 예상 에러 코드(400/501 등)가 돌아오는지 확인한다.

**Frontend UI 변경 시:**
- backend dev + frontend dev를 모두 띄운다.
- **변경된 UI 화면에 접근하는 API를 직접 호출**하거나, 해당 컴포넌트가 사용하는 데이터를 API로 확인해 렌더링에 필요한 데이터가 정상인지 검증한다.
- UI 컴포넌트 구조 변경(새 섹션, 그룹 헤더, 버튼 추가 등)은 해당 컴포넌트의 렌더링 로직을 코드 리뷰로 재확인한다:
  - 조건부 렌더링 분기가 의도대로인지 (예: `group !== prevGroup` 비교, `description &&` 가드)
  - 새로 추가한 JSX 요소가 올바르게 닫혀 있는지
  - key prop이 고유한지
  - Fragment 사용 시 올바르게 감싸고 있는지
- **가능하면** 브라우저를 직접 열어 화면을 확인한다. 브라우저 접근이 불가하면 위 코드 리뷰 + API 검증으로 대체하되, **"브라우저 시각 확인은 사용자에게 위임"**을 명시한다.

**DB 스키마/데이터 변경 시:**
- 변경 후 실제 DB에 쿼리를 날려 스키마/데이터가 기대 상태인지 확인한다.

### 검증 보고 양식 (agent_log.md / 채팅 보고 시 필수)
변경 완료를 보고할 때 아래 테이블을 반드시 포함합니다:

```
| 검증 계층 | 결과 | 비고 |
|-----------|------|------|
| 정적 분석 | ✅/❌ | (에러 내용 또는 "0 errors") |
| 빌드 | ✅/❌ | (frontend/backend 각각) |
| 자동 테스트 | ✅/❌ | (N/N pass) |
| 런타임 통합 | ✅/❌ | (어떤 endpoint/화면을 확인했는지) |
```

- 런타임 통합 칸에 "미실시"를 적는 것은 **금지**. 반드시 실행하고 결과를 기록한다.
- 브라우저 시각 확인이 불가할 경우에만 "코드 리뷰 + API 검증으로 대체, 시각 확인은 사용자 위임"으로 적는다.
...
- **세부 단계 테이블 상태 표기 — 엄격 기준 (필수):**
  ...
  **상태 이모지 적용 위치 (3곳 일관성 규칙):**
  ...
  3. **세부 단계 테이블** — 테이블에 `| 상태 |` 컬럼을 추가하고 각 행에 이모지 기입
...
- **plan.md 작성 규칙(plan.md 생성/수정 시 반드시 준수):**
  ...
  - **언어 규칙:**
    - `plan.md`는 **한국어 중심**으로 작성합니다.
    - API, CLI, config, diff, log, status, hook 같은 일반적인 영어 용어는 굳이 어색하게 번역하지 않아도 됩니다.
    - EN/KO 병기, 영어 우선 배치, 정합성(parity) 유지 규칙은 적용하지 않습니다.
