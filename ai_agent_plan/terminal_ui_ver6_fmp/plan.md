...
> 이 문서는 2026-03-20 기준 `terminal_ui_ver6_fmp` 작업의 구현 계획서다. 현재 목표는 뉴스 UI와 backend에 `FMP press release` 전용 흐름을 추가하는 것이다.
...
- 현재 UI의 source type 필터는 `all`, `company_news`, `press_release`, `sec_filing`, `market_news`만 있다.
...
- 변경 파일 기준 정적 에러 검사는 모두 통과했다.
- `npm.cmd run build`로 backend build를 확인했다.
- 남은 검증은 사용자가 직접 UI에서 버튼을 눌러 동작을 확인하는 수동 점검이다.