...
| 가격 반응 (전체 change) | 해당 뉴스의 첫 반응 거래일 기준 **모든 change 컬럼**을 테이블로 기록한다. 장 후(16:00 이후) 발표면 다음 거래일 기준. 컬럼: `Change_1d_Pct`, `Change_From_Open_Pct`, `Change_7d_Pct`, `Change_14d_Pct`, `Change_30d_Pct`, `Turnover`. |
| 다음 거래일 반응 | 첫 반응 거래일 바로 다음 거래일의 동일 change 컬럼 전체 |
| 직전 어닝 / 다음 어닝 | 해당 이벤트 시점 기준 **가장 최근 과거 어닝 날짜** + **가장 가까운 미래 어닝 날짜**를 함께 기록한다. (상세는 아래 "어닝 날짜 연동 규칙" 참고) |
| 해석 | 이 이슈가 바스켓 전체에 미친 영향, 스필오버 여부, 지속성. 7d/14d/30d change로 며칠간 유지됐는지 판단. Turnover 급등은 기관/대형 자금 유입 가능성을 시사. |
...
- **해석 가이드**: 1d_Pct는 당일 반응 크기, 7d/14d_Pct는 이슈의 지속력, 30d_Pct는 이슈가 추세를 바꿨는지, Turnover 급등은 기관 자금 유입 시그널로 해석한다.
...
| 가격 반응 (전체 change) | 바스켓 내 주요 종목의 당일 **전체 change 컬럼** 테이블: `Change_1d_Pct`, `Change_From_Open_Pct`, `Change_7d_Pct`, `Change_14d_Pct`, `Change_30d_Pct`, `Turnover`. 이전 파동 대비 7d/14d가 어떻게 변했는지도 비교. |
| 의미 | 이 이벤트가 이슈 확산에서 어떤 역할을 했는지. Turnover 변화로 자금 유입/이탈 판단. |
...
- 조사 범위: 최소 1년 소급.
- 비교 항목: 촉매 성격, **전체 change 컬럼** (`1d`, `From_Open`, `7d`, `14d`, `30d`, `Turnover`), 지속 기간, 후속 어닝에서의 확인 여부.
- 7d/14d/30d 비교를 통해 "과거 사례는 1주 만에 되돌렸지만 이번에는 30d까지 유지" 같은 지속성 비교가 가능하다.

### 교차 티커 비교 (cross-ticker comparable)
- 다른 이슈 티어에서 유사한 패턴(대형 계약 발표 → 밸류체인 확산 등)을 보인 사례를 조사한다.
- 비교 항목: 이슈 성격, 바스켓 크기, 확산 속도, 티어 판정 결과, **전체 change 컬럼 비교** (특히 7d/14d/30d로 파동 지속력 교차 비교).
...
- 허용되는 소스 태그: `company_news`, `fmp_pr`, `fmp_sec`, `calendar_events`, `ohlc_1d`, `investing`, `company_profiles`.
...
1. **가격 반응 없이 정당성 판정 금지**: 등락률 데이터 없이 "주가 영향이 있었다/없었다"를 판단하지 않는다.
...
4. **어닝 날짜 누락 금지**: 확산 타임라인의 각 이벤트에서 다음 어닝 날짜를 조회하지 않으면 미완료로 간주한다.
