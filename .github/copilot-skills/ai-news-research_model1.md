...
- 날짜별 `Current-News 전수 스크리닝 요약` 또는 그에 준하는 **상단 요약 표**를 만들 때는, 가능하면 후보별로 아래 수급 컬럼을 함께 넣는다.
  ...
  - `수급 가산점`
  - `최종 등급`
- 즉 상단 표는 필요하면 기존 `Ticker / mcap / Industry / Headline / 분류 / 근거`만으로 끝내지 말고, 최소한 final 경쟁 후보들에 대해서는 `float %`, `institutional %`, `insider %`, `가산점`, `최종 등급`이 보이도록 확장한다.
...
| Ticker | mcap | Float % | Institutional % | Insider % | 수급 가산점 | 최종 등급 | 근거 |
|--------|------|---------|------------------|-----------|-------------|-----------|------|
| ABCD | 2.4B | 82.7% | 24.1% | 11.4% | ⭐⭐⭐ Strong | A | 후기 임상 positive + same-ticker 반응 우세 |
| EFGH | 5.8B | 61.3% | 47.8% | N/A | ⭐⭐ Medium | A- | strong contract이나 선반영 가능성 존재 |
| IJKL | 1.1B | N/A | N/A | N/A | ❔ Pending | 보류 | 수급 데이터 미확보로 provisional |

- 사용자가 “처음 표”, “상단 요약 표”, “날짜별 스크리닝 표”를 빠르게 보고 판단하는 상황을 전제로, 이 표에서는 **수급/등급을 한 줄에 압축해서 보여주고**, 상세 계산 근거는 아래 개별 ticker 본문으로 내려보낸다.
