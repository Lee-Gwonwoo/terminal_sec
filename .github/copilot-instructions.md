...
- 위 `ai-news-research.md`가 적용되는 작업에서는, 그 문서의 `plan / log 문서 작성 예외`를 본 섹션보다 우선 적용한다. 즉 AI 뉴스 리서치/분석 자체만 수행하는 작업은 채팅에 세부 계획을 설명할 수는 있어도 `ai_agent_plan/.../plan.md`, `agent_log.md` 생성·갱신 의무가 없다.
...
- **Docs / “prompt” Markdown 규칙(중요)**
  ...
  - **상세 작성 규칙(간략 금지):** 문서는 동작을 충분히 설명해야 합니다.
    ...
    - 사용자가 조사형 산출물의 **예시 형식**을 제시했다면, 특별한 충돌이 없는 한 그 예시의 섹션 순서, 표 중심 구성, 핵심 요약 방식부터 우선 반영합니다.
    - 특히 `유상증자`, `오퍼링 이력`, `ATM / registered direct / PP / follow-on history`, `희석 이력`처럼 회사별 자본조달 타임라인을 정리하는 요청에서는 **한 회사씩 완료**를 기본으로 하고, `전체 목록 표 -> 주요 인사이트 -> 필요 시 비교/해석` 순서를 우선 사용합니다.
    - 거버넌스 비교와 오퍼링 history가 함께 필요한 조사형 산출물에서는 `거버넌스 표/요약 -> 회사별 오퍼링 이력 -> 마지막 비교/순위` 순서를 우선 사용합니다. 거버넌스 표에서 ATM/희석을 이미 요약했더라도 회사별 오퍼링 history 표는 생략하지 않습니다.
    - 사용자가 `5개 종목 Class B / Dual-class 구조 조사 결과`, `지분구조 및 희석 이력 종합 정리`, `각 종목별 세부 분석`, `비교 업데이트`, `거버넌스 관점 순위` 같은 섹션명을 직접 제시했다면, 특별한 충돌이 없는 한 그 제목 체계와 표 중심 흐름을 유지합니다.
...
- [AI 뉴스 리서치 (공통)](copilot-skills/ai-news-research.md)
  ...
  - [Model_100_issue_analysis](copilot-skills/ai-news-research_model100_issue_analysis.md)
  - [Model_200_earning_lists](copilot-skills/ai-news-research_model200_earning_lists.md)
