...
**Model_1 단계별 실행 기본 모드 (필수)**

- `Model_1`은 기본적으로 `원샷 일괄 수행`보다 **단계별 실행**을 기본값으로 둔다.
- 사용자가 `한 번에 다 해줘`, `full model1으로 끝까지 해줘`처럼 요청하더라도, 에이전트 내부 실행 순서는 반드시 아래처럼 쪼갠다.
  1. `1단계`: current-news 넓은 스크리닝으로 후보군을 먼저 확정한다.
  2. `중간 저장`: 1단계 후보 표를 research page에 먼저 기록한다.
  3. `오너쉽 수치 채움`: `GET /api/tickers`로 후보군의 `market_cap`, `float %`, `institutional %`, `insider %`를 붙인다.
  4. `2단계`: 후보군에 대해서만 same-ticker / other-ticker 유사사례를 조사한다.
  5. `3단계`: 유사사례 분포 + 시총 맥락 + 오너쉽 가감점으로 최종 등급을 닫는다.
- 즉 `1단계 후보 셋`이 아직 흔들리는 상태에서 바로 2단계와 3단계까지 밀어붙이면, 검색 범위와 최종 ranking이 계속 흔들리므로 **후보군을 먼저 잠그고(candidate freeze), 그 상태를 page에 남긴 뒤** 다음 단계로 넘어가는 것을 기본 운영으로 본다.
- 사용자가 page id를 주었거나 기존 research page 문맥에서 `Model_1`을 요청한 경우, **1단계가 끝나는 즉시 해당 page 본문에 후보 리스트를 먼저 저장**한다. 최종 결론이 아직 없더라도 저장을 미루지 않는다.
- page가 아직 없지만 사용자가 채팅 밖에서도 이어서 검토할 수 있는 산출물을 원하면, **2단계 진입 전에 사용할 research page를 먼저 정한 뒤** 1단계 후보를 거기에 저장한다.
- 1단계 저장본은 최종판이 아니므로, 제목 또는 섹션 상태에 `1단계 완료`, `provisional`, `후보 확정`, `오너쉽 수치 반영 전` 같은 진행 상태를 명시한다.
- 따라서 실무적으로는 `Model_1`을 한 번에 길게 요청하는 방식보다, `1단계 후보 확정 -> page 저장 -> 2단계/3단계 후속 조사` 흐름으로 운영하는 편이 기본값에 가깝다.
...
**Model_1 시가총액 해석 가드레일 (필수)**

- `Model_1`은 **시가총액 고정 컷오프를 직접 분석 대상 선정 조건으로 사용하지 않는다.** large-cap, mega-cap ticker도 사건 강도와 유사사례 증거가 충분하면 직접 분석 대상이 될 수 있다.
- 다만 시가총액은 여전히 **반응 크기 해석과 비교 사례 보정의 핵심 맥락**이다. 같은 사건이라도 small-cap과 mega-cap은 반응 빈도, 속도, 상한이 구조적으로 다를 수 있으므로 이를 설명 없이 섞으면 안 된다.
- 따라서 `other-ticker` 사례를 쓸 때는 각 비교 ticker의 `market_cap`을 같이 적고, 현재 ticker와의 시총 차이가 반응 해석에 어떤 왜곡을 만들 수 있는지 짧게 메모한다.
- large-cap / mega-cap 현재 뉴스를 직접 분석할 때도 허용되는 것은 **분석 대상 포함**이지, 소형주 급등 사례의 반응 크기를 그대로 이식하는 것이 아니다. 시총이 클수록 개별 기사 1건의 재평가 상한, 선반영 가능성, 분산된 수급 구조를 더 보수적으로 해석한다.
...
- `institutional ownership % >= 90` 인 ticker는 기본적으로 `최종 중요 뉴스 리스트`, `same-ticker 유사사례 상세 분석`, `other-ticker 비교를 포함한 상세 본문`의 직접 분석 대상에서 제외한다.
...
- **감점 밴드 (- 방향):**
  - `🔻🔻🔻 강한 감점 (-)`: `float % < 30` 이거나 (`institutional % >= 80` 이고 `institutional % < 90`) 이거나 `insider % >= 40`
...
- 1단계(넓은 스크리닝)를 끝낸 직후, **2단계로 넘어가기 전에** 아래 두 가지를 반드시 수행한다.
  1. **1단계 후보 목록을 research page에 중간 저장한다.** 이렇게 하면 작업이 중단되거나 대화가 끊어져도 후보 리스트가 남아 있어, 이후 오너쉽 반영이나 2/3단계를 이어서 할 수 있다.
    ...
    - **원샷 요청이더라도 예외 없이 이 중간 저장을 먼저 한다.** 즉 1단계 후보 표를 page에 남기기 전에는 2단계 유사사례 분포 표나 3단계 최종 ranking을 먼저 닫지 않는다.
    - 이미 같은 날짜/페이지에서 분석을 진행 중이라면, **새 page를 만들기보다 같은 research page 본문을 갱신**해 단계별 상태가 한곳에 누적되게 한다.
  2. **`GET /api/tickers`를 호출해 1단계 후보 전체(primary, secondary, watch 포함)의 오너쉽 데이터를 조회한다.**
     - 조회 결과를 채팅에 표로 보고하고, DB에 값이 있는 ticker는 **상단 요약 표의 Float%/Inst%/Insider% 칸을 바로 채운 뒤** research page를 다시 저장한다.
      - 이때 `institutional % >= 90` 인 ticker는 `제외 (Inst 90%+)`로 즉시 표시하고, 2단계/3단계 직접 분석 대상으로 넘기지 않는다.
