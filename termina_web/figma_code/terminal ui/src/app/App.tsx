...
export default function App() {
  ...
  const [linkedTicker, setLinkedTicker] = useState<{ [linkId: number]: string }>({});
  ...
  const handleStartTab = (selectedWindows: WindowType[]) => {
    ...
    if (windowCount === 1) {
      positions.push({ top: 20, left: 20, width: 800, height: 600 });
    } else if (windowCount === 2) {
      // Side by side
      positions.push({ top: 20, left: 20, width: 600, height: 600 });
      positions.push({ top: 20, left: 640, width: 600, height: 600 });
    } else {
      // Staggered layout
    ...
    const newWindows: WindowInstance[] = selectedWindows.map((type, index) => ({
  ...
  const handleCloseWindow = (windowId: string) => {
    ...
    setTabs(tabs.map(tab => 
      tab.id === activeTabId
        ? { ...tab, windows: tab.windows.filter(w => w.id !== windowId) }
        : tab
    ));
  };

  const handleTickerClick = (ticker: string, linkId?: number) => {
    if (linkId) {
    ...
    <div className="h-screen flex flex-col bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100">
      ...
      <div className="h-12 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between px-4">
        <div className="flex items-center gap-2">
          <h1 className="text-lg font-medium">Stock News Platform</h1>
        </div>
        ...
        {activeTab && activeTab.windows.length === 0 ? (
          ...
          activeTab?.windows.map((window) => (
            <DraggableWindow
              ...
              onTickerClick={(ticker) => handleTickerClick(ticker, window.linkId)}
              initialTicker={window.linkId ? linkedTicker[window.linkId] : undefined}
            />
          ))
        )}
      </div>
